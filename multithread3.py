            ...
            try:
                if ball[0] is not None and ball[1] > 20:
                    ...
                    if korv[0] is not None:
                        ...
                        if flag == 0:
                            #print("türa")
                            omni3.ballRotate(values,
                                             -1 * omni3.pidBallCenterRotateSpeed(korv[0], integral, derivative,
                                                                                 err_prev_rot, errors_array),
                                             omni3.pidBallCenter(x, integral, derivative, err_prev),
                                             omni3.pidBallCenterForward(y, integral, derivative, err_prev_fwd))
                        ...
                        if all(abs(n) < 20 for n in errors_array) and y > 540 or flag == 1:
                            ...
                            if y < 710:
                                print("lammas")
                                omni3.ballRotate(values,
                                                 -1 * omni3.pidBallCenterRotateSpeed(korv[0], integral, derivative, err_prev_rot, dummy_errors_array),
                                                 omni3.pidBallCenter(x, integral, derivative, err_prev),
                                                 -5)
                            else:
                                print("lehm")
                                ...
                                while x < 4000:
                                    ...
                                    x -= -1
                                flag = 0
                                omni3.endThrow(values)
                    # kui pall on kaugemal kui väärtus
                    elif y < 60 and y is not None and y != 0:
                        #print(1)
                        pid = omni3.pid2(x, integral, derivative, err_prev)
                        omni3.toBall(values, 30, [pid, y])
                    elif y < 230 and y is not None and y != 0:
                        #print(1)
                        pid = omni3.pid2(x, integral, derivative, err_prev)
                        omni3.toBall(values, 55, [pid, y])
                        # kui pall on kaugemal kui väärtus
                        #print("uss")
                    elif y < 440 and y is not None and y != 0:
                        #print(1)
                        pid = omni3.pid2(x, integral, derivative, err_prev)
                        omni3.toBall(values, 25, [pid, y])
                        #print("lammas")
                    elif y > 440 and y is not None and y != 0:
                        #print(1)
                        #print(ball)
                        omni3.ballRotate(values,
                                         -1 * omni3.pidBallCenterRotateSpeed(korv[0], integral, derivative, err_prev_rot, errors_array),
                                         omni3.pidBallCenter(x, integral, derivative, err_prev),
                                         omni3.pidBallCenterForward(y, integral, derivative, err_prev_fwd))
                        #print(errors_array)
